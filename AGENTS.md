...
- The task workflow and its approval gates apply only to selected work from `TASKS.md`. Perform small, direct, low-risk changes immediately without a plan, intelligence-level recommendation, or approval gate; keep them uncommitted in the current changeset until the user requests or approves Git operations. Automatic commit and push applies only to selected `TASKS.md` tasks.
