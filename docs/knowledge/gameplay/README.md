# Gameplay knowledge

This domain owns gameplay-behavior findings that do not belong to localization,
runtime infrastructure, or one feature's canonical patch data. Each document
is organized by gameplay domain and keeps unresolved leads beside the related
established knowledge.

Historical unresolved leads must have their labels and effects re-established
from current disassembly or runtime evidence before implementation.

- [Battle behavior](battle.md)
- [Battle AI](battle_ai.md)
- [Battle action-command and input interpretation](action_commands.md)
- [Battle status effects and item-effect lifecycle](status_effects.md)
- [Battle-session and stage lifecycle](battle_lifecycle.md)
- [Battle camera control](battle_camera.md)
- [Battle-entity ownership and lifecycle](battle_entities.md)
- [Collision and hit-query infrastructure](collision.md)
- [Battle hit-response state](hit_response.md)
- [Native chakra and guard state](chakra_and_guard.md)
- [Awakening and transformation gameplay](awakening.md)
- [Projectile gameplay lifecycle](projectiles.md)
- [Battle support mechanics](support_mechanics.md)
- [Battle stage gameplay knowledge](stages.md)
- [Match termination and outcome flow](match_outcomes.md)
- [Practice-mode architecture](practice_mode.md)
- [Pause, start-menu, and battle-restart control](pause_and_replay.md)
- [NA228 modes and menu navigation](modes_and_navigation.md)
- [Character identity in battle](character_ids.md)
- [60 FPS implementation research](framerate.md)
- [Storm-style substitution bar](substitution_bar.md)
- [Substitution knowledge](substitution.md)
- [X-dash knowledge](xdash.md)
