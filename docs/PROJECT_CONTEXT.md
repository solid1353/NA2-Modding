...
- Root `_na2.ps1` is the only routine user-facing entrypoint. Bare `na2`, `na2 -b`, bare `na2 -t`, `na2 -c`, `na2 -p`, and `na2 act` retain their Current/Previous/Candidate behavior. `na2 -t work/<task title>/build/<name>.iso` adds a full verified worker-output mode with task-owned staging/logs and no shared-state mutation; agents must use that form for builds.
