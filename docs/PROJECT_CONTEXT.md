...
- `na2_patcher/`: profile schema v2, ordered feature/module orchestration, hash-pinned module data, and the translation/raw-binary/UI-texture/external-translation/disc-identity implementations. `na2_patcher/profiles/current/` defines enabled features separately from ordered modules and binds them through module-specific selections; active raw-binary modules currently use group selections only. Raw-binary and UI-texture hashes exclude adjacent READMEs and engine code; the complete committed checkpoint pins the integrated implementation. The disc-identity module runs last and performs the declared equal-length `SLPS_258.37` to `SLPS_222.28` boot-path rename.
