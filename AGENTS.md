...
- Keep reusable executable engines, schemas, and tools under `na2_patcher/modules/`. Keep reproducible feature-owned module inputs as hash-pinned declarative data under the owning `na2_patcher/features/<feature>/` directory; retain retired inputs through Git history and annotated checkpoint tags.
...
- Treat `na2_patcher/features/translation/translation_importer/mappings.tsv` as the translation feature's current versioned importer input. Profiles may reference it only with an exact content hash; changing it requires updating the profile pin as an explicit translation version change.
- The normal profile workflow invokes `translation_importer` to validate and import official strings, then `string_patcher` compiles those imports with its local declarations and delegates all guarded writes to `binary_patcher`. The profile log records the import plan and shared binary patch results. There is no standalone translation-export command or source-hash bypass.
