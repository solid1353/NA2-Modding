...
- For every agent-authored Git commit, override the author for that commit only with the agent's normal name and a normalized `<agent-name>@agent.invalid` address (for example, `Codex <codex@agent.invalid>`). Do not change repository or global Git identity, rewrite commit subjects, or alter user-authored commits.
