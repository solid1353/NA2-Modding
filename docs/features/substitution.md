...
This document owns the substitution gauge's detailed runtime architecture,
builder integration, and validation. The public settings contract is owned by
...
## Storm-series design evidence

### Evidence labels

- **Official later-game behavior** is stated by a publisher-provided source.
- **Measured later-game behavior** comes from community measurement or visual
  inspection and is not promoted to an official specification.
- **Confirmed NA2 evidence** comes from clean binaries, maintained disassembly,
  or recorded runtime observations.
- **Inference** follows from confirmed evidence but remains subject to runtime
  validation where noted.

### Storm-series behavior
...
community measurement and are not native NA2 facts.
...
discrepancy therefore must not be copied as an NA2 rule. Any NA2 adaptation
must express timing in native battle-time counts at its fixed 30 FPS cadence.
...
later-game timing evidence, but an NA2 adaptation must use proven battle-clock
gates rather than a list of guessed animation states.
...
capsules. The transferable information is the presentation hierarchy and
behavior, not another game's texture artwork.
...
| Stock size | `25.0` | Four equal gauge intervals |
...
hook shims. Capacity and stock size remain fixed together for the parity mode;
tuning normally changes delay, natural rate, or damage recovery scale.
...
The catalog encoding makes the first three products exact integers. At
...
Each player slot maintains `meter_counts`, `recovery_delay_counts`,
`damage_recovery_remainder`, `last_hp_q16`, and identity/reset metadata. Native
HP is converted once per sample with
...
`last_hp_q16` is initialized without recovering gauge when a slot first binds.
Healing or an explicit HP restoration raises the baseline without creating
received damage. Damage taken while a cinematic temporarily suspends ordinary
fighter updates is captured as the net HP decrease on the next valid update.
HP sampling and damage recovery are independent of the battle-clock callback, so
local hit-stop does not duplicate recovery or become an invented recovery
clock rule.
...
Damage is not banked while the meter is full. Otherwise a player could take
damage at full gauge, spend later, and receive an immediate hidden stock that
the later-game UI never communicated. Integer remainder carry makes recovery
proportional and deterministic, and a large HP decrease can recover more than
one stock. The community source establishes the `31.25` relationship but does
not publish these rounding details; they are NA2 implementation behavior.
...
The boot-ELF and BTL identities, load bases, and file/export conversions follow
[Standard game file identities](../knowledge/game/files/file_identities.md).
Raw offsets below use those conventions.
...
NA2.28 owns a guarded hook at raw ELF offset `0x14DB80` for the shared
`settings_fighter_update_shim`. It receives the live fighter pointer, runs the
X-dash charge check before the displaced native fighter update, and applies the
shared Chakra mode afterward. Adding a second independent hook at the same
instruction would conflict.
...
delay-slot bytes are `B8251C0C00000000`. The guarded wrapper at this call:
...
between updates, and cannot recover twice from the same stored value. Reset and
...
recovery from damage but do not publish source filters. The implementation uses
this inclusive rule and has two explicit blind spots:
...
## Resident state and ownership

The resident payload keeps two player slots instead of adding fields to native
fighter objects. Each slot stores its fighter, integer meter and recovery delay,
damage-recovery remainder, latest quantized HP sample, battle generation, and
...
BTL support-controller hook ------------------cache-> controller and overlay `$gp`
BTL top-HUD hook -------------------------------draw-> mirrors through shared layout
```
...
its resident sprite primitives. The implementation places each bar at mirrored
X offset `66.5` and Y offset `39.5` from the shared top-HUD layout, owns its
fill width, cost-marker position, visibility, and draw order, and shifts both
Battle HUD character names down by 11 logical units in Gauge mode. Recovery
uses four equal increments, but spending is per-character and the bar displays
aggregate `0.0..1.0` fill continuously instead of drawing four new cells. The
complete native support renderer, including its coordinates, button badge, and
...
| `+0x0B` | fill/animation state: `0` insufficient support, `1` usable below full, `2` full |
...
native full color. A zero-cost action is affordable even at zero meter. Color
communicates resource availability; the other native substitution gates remain
independent.

The full-width inner background uses the native neutral tint. The cost marker
uses the native resident tint through the same RGB packing as the fill, while
retaining the scaled rectangle primitive and top-HUD alpha. No support-gauge
state is borrowed to decide substitution availability.

The signed load displacement resolves the marker's native neutral RGB tint as
`(0x80,0x80,0x80)`.
Static tracing establishes the corrected data source and primitive behavior;
the resulting colors still require visual acceptance.

### Sprite geometry and state ownership

The same clean renderer loads Player 1/Player 2 X bases `120.0/392.0` at BTL
raw `0x68C24/0x68C88` and shared Y base `340.0` at `0x68C6C`; the implementation
...
as BTL `$gp - 0x5CD8`, `$gp - 0x5CD0`, and `$gp - 0x5CC0`. The implementation
...
untouched. The implementation replaces only the following multiplication
...
The implementation uses BTL `0x69380` only to capture BTL `$gp`, call live
...
The renderer computes its anchors as layout X plus mirrored `66.5 * scale` and
layout Y plus `39.5 * scale`, applies the same scale to every offset and
dimension, and uses the primary HUD sprite's current alpha. This attaches the
...
top-HUD gate, and the later native support call remains suppressed. The feature
does not use the BTL `0x6745C` post-chakra hook, support-controller mode word,
cross-module fill buffer, native marker hook, or any substitution-specific edit
inside the support renderer.

The builder applies the hook through a guarded BTL target rather than a fixed
PNACH write because the overlay loads and unloads. Static evidence establishes the dataflow
...
Omitting `gauge` retains those defaults. Capacity `100`, stock size `25`,
...
and emits one aligned read-only fragment:
...
The reader converts resolved numeric values through their decimal string form
and performs validation and packing with `Decimal`. It requires
...
non-overlap and both selection outcomes.
...
The following behavior still requires runtime confirmation for both P1 and
COM/P2:
...
| Top-HUD placement | Independent textured bars use mirrored X offset `66.5` and Y offset `39.5` from each side's shared HUD layout without overlapping the shifted names |
| Battle HUD character names | Gauge mode retains the native X path and adds `11.0` to the current Y; atlas rectangles and width fitting remain unchanged |
...
| Linked Attack | Retains its control mapping; the shared Support row controls field support while selected support data remains available to linked Jutsu |
...
These details remain unresolved without runtime evidence:
