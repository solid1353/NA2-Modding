...
inventories and their provenance.

## Research coverage

- **Assigned scope:** The original NA2 disc layout and the roles of its root
  files, data archives, IOP modules, EE overlays, movies, and major CCS
  resource families.
- **Exploration depth:** The read-only extraction was inspected through headers,
  strings, archive and media metadata, executable references, same-name
...
All four files are byte-identical between NA2 and NUN5.
