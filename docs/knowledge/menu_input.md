...
This record preserves runtime behavior that was previously available only in dated raw-binary patcher logs. The canonical per-screen results are in `na2_patcher/modules/raw_binary/patch_sets/menu_input/runtime_tests.tsv`.
...
- `runtime_tests.tsv` records the complete four-test runtime matrix.

These module files are the canonical evidence. This document is the interpretation and navigation layer, not a second patch definition.
