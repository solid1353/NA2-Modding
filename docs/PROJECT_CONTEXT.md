...
- `translation_package_builder/mappings.tsv` is the live translation workspace and `na2 tr` remains a standalone TSV export. Reproducible builds use immutable mapping snapshots under `na2_patcher/milestones/`; the current profile references translation m03/v33. Legacy builder-archive milestones m01/v25, m02/v30, and m03/v33 remain frozen provenance under `milestones/packages/` and are not modified.
