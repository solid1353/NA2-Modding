...
- Keep active package history under `packages/`, including packages generated by Codex or project scripts as well as downloaded/imported inputs. Use unique versioned filenames and do not overwrite prior packages.
- Keep frozen reusable package milestones under `milestones/packages/`; do not mix ordinary test packages into it. Name them `NA2_MILESTONE__<TYPE>__<VERSION>__<YYYYMMDD_HHMMSS>__<MEANING>.<ext>` so type/version references remain stable and timestamps preserve chronology.
- Treat the translation builder and its generated TSV as live, replaceable inputs. Never hardcode a builder or translation version as permanently current; inspect the installed builder and latest generated run each time.
- Keep frozen milestone artifacts under the root `releases/` link.
...
- New root folders are allowed when useful, but every new root folder must be described in `AGENTS.md` and `docs/PROJECT_CONTEXT.md`.
...
For full project milestones, place new frozen files in `releases/` with clear version/postfix names. Active working files stay in `build/`.
