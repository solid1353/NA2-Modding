...
- New translation milestones freeze immutable canonical mapping data plus a hash-pinned profile; the integrated translation engine is versioned with the repository. Legacy complete builder archives remain frozen provenance. Generated translation TSVs are compatibility run outputs, not milestones.
- Treat `translation_package_builder/mappings.tsv` as the live replaceable translation workspace. Reproducible builds must reference an immutable snapshot under `na2_patcher/milestones/`, never the live table.
- The standalone translation builder is a compatibility/export interface. The normal profile workflow invokes the integrated translation module directly and records its generated plan under the profile run log.
