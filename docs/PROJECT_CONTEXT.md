...
- `na2_patcher/modules/string_patcher/`: the reusable semantic fixed-size string engine. Translation-owned declarations live under `na2_patcher/features/translation/string_patcher/`; the engine merges them with runtime imports, compiles one in-memory binary-patcher package, and delegates byte guards, conflict validation, composition, and logging to `binary_patcher`.
