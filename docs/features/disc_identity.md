...
The rejected serial alternatives are:
...
Root `game.json` declares the product title and explicit output boot path.
...
GameDB, the Game List falls back to the scanned image filename. Because normal
project images are intentionally named
...
can retain `Narutimate Accel v2.28 - Latest`. A direct
...
`SLPS-22228 [?]` for an earlier modified project identity. The active
`SLOP-NA228` identity has not been separately
...
The project-owned serial-wide cheat file is
`@pcsx2_files/games/NA228/NA228.pnach`. Configured build launches pass it
explicitly to PCSX2. It applies across CRC changes; optional group-level
`crc =` metadata limits individual groups. The retained Current,
