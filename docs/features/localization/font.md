...
- `localization__font__numeric_formatting__save_load` routes only the six Save/Load numeric blocks
  ...
  patch.
- `localization__font__numeric_formatting__battle_settings` routes only the ordinary Battle Settings
...
- `localization__font__numeric_formatting__ninja_song` redirects exactly five guarded BTL formatter
...
The separate `localization__font__layout__global_selected_style` patch changes NA2's complete
...
The first thin caller layer, `localization__font__layout__controls`, redirects the shared
...
The second thin caller layer, `localization__font__layout__titles`, replaces only two guarded BTL
...
The dedicated Pause Controls layer, `localization__font__layout__pause_controls`, replaces the
...
The dedicated ss4 layer, `localization__font__layout__quit_confirmation`, authors no newline in
...
The separate `localization__font__layout__mode_select_confirmation` caller layer reuses the
...
retains `v2_quit_choices_scope`, whose top Yes/No result was
...
The separate `localization__font__layout__collection_confirmation` layer owns three clean ETC
calls. Files `0x12680` and `0x148C8` are the two body consumers and both route
through `v2_collection_body_adapter`; file `0x126A0` scopes
only the complete choice object through
`v2_quit_choices_scope`. Live tracing proved the later
...
The separate `localization__font__layout__collection_lists` layer owns the shared Collection
...
The `localization__font__layout__character_select_modal` layer owns four exact main-ELF
...
`v2_quit_choices_scope`, so its rows receive the same
...
The declarative `localization__font__layout__linked_mode_modal` layer owns only main-ELF
...
The separate `localization__font__layout__jutsu_selector` layer owns only the final Jutsu-row
...
The separate `localization__font__layout__special_controls_body` layer completes the explanatory
...
The third thin caller layer, `localization__font__layout__practice_explanations`, replaces only the
...
`ninja_song_ascii_number` symbol and all five Ninja Song BTL
