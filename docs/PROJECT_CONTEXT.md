...
- `na2_patcher/features/localization/translation_importer/mappings.tsv` is the Translation feature's current hash-pinned v35 importer input. The current profile imports and validates all official-string targets first; `string_patcher` then compiles every enabled import with its local declarations and delegates their guarded writes to `binary_patcher`. No standalone export or source-hash bypass exists.
...
- `na2_patcher/modules/translation_importer/`: the reusable official-string importer engine. Translation-owned mappings live under `na2_patcher/features/localization/translation_importer/`.
...
- `na2_patcher/modules/texture_patcher/`: the reusable fixed-size texture derivation engine. The Translation feature owns its 34 source-derived NUN5 UI recipes under `na2_patcher/features/localization/texture_patcher/`; no replacement blobs are stored.
