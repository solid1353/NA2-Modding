...
- Former PNACH sections preserved as raw-binary patch sets are `Testing`, `Rendering`, `QoL`, and `Battle logic`. Raw-binary schema v2 organizes each package as groups, atomic patches, and exact edits; `default_enabled` preserves historical state. Rendering is currently an empty disabled feature and module in the active profile.
