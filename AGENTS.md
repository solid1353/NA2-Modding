...
- Treat `na2_patcher/modules/translation_importer/mappings.tsv` as the translation importer's current versioned input. Profiles may reference it only with an exact content hash; changing it requires updating the profile pin as an explicit translation version change.
