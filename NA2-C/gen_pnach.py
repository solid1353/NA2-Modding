...
CRC = "C0659AD1"
PNACH_FILE = f"C:/Users/zMath3usMSF/Documents/PCSX2/cheats/{CRC}.pnach"
ELF = "data/FILES/SLES_556.05"
PS2DEV = "/c/msys/1.0/local/ps2dev/ee/bin"
PS2SDK = "/c/msys/1.0/local/ps2dev/ps2sdk/bin"
OBJ_DIR = "obj"
BUILD_DIR = "build"

HEADER_FILE = "src/Main.h"

os.environ["PATH"] = f"{PS2DEV}:{PS2SDK}:" + os.environ["PATH"]
    ...
    def resolve_tokens(token_str):
        ...
        for token in tokens:
            if token.startswith(("0x", "0X")):
                ...
                print(f"  AVISO: label '{token}' nao encontrada em .org")
...
def parse_header_symbols(header_path):
    """Le o .h gerado pelo generate_header.py e retorna um dict
    {nome_da_funcao: endereco_int}, extraido do comentario "// 0xENDERECO"
    que fica do lado de cada declaracao extern. Isso evita ter que manter
    na mao um .definelabel pra cada funcao existente do jogo -- o
    endereco ja esta ali do lado da assinatura, o build so precisa ler.
    """
    symbols = {}
    if not os.path.exists(header_path):
        print(f"  AVISO: header '{header_path}' nao encontrado -- "
              f"nenhum simbolo sera carregado dele")
        return symbols
    ...
    with open(header_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if name in symbols and symbols[name] != addr:
                print(f"  AVISO: '{name}' aparece 2x no header com enderecos "
                      f"diferentes ({hex(symbols[name])} e {hex(addr)}) -- "
...
def collect_all_symbols(obj_addrs):
    """Retorna dict {nome_simbolo: endereco_final} para todas as funções de todos os .o"""
    symbols = {}
...
def generate_linker(filename, labels, importobjs, word_orgs, asm_blocks,
    ...
    for _, name in importobjs:
        ...
        rodata_addr = (data_addr + data_size + 7) & ~7
        obj_rodata_addrs[name] = rodata_addr

        bss_size = obj_bss_sizes.get(name, 0)
        bss_addr = (rodata_addr + rodata_size + 7) & ~7

        addr = (bss_addr + bss_size + 15) & ~15
    ...
    print(f"  build/linker_generated.asm gerado")
    for name in obj_addrs:
        print(f"    {name} -> text={hex(obj_addrs[name])} rodata={hex(obj_rodata_addrs[name])}")
    for addr, count in asm_blocks:
        print(f"    asm block -> {hex(addr)} ({count} instruções)")
...
def main():
    ...
    print(f"Carregados {len(header_symbols)} simbolos de {HEADER_FILE}")
    ...
    if not base_address:
        print("ERRO: BASE_ADDRESS não encontrado no linker.asm")
        return
    ...
    for _, name in importobjs:
        ...
        if run_cmd(f"ee-gcc -w -D_EE -G0 -O2 -std=c99 -c {src} -o {obj}") != 0:
            return
        objs.append(name)
    t1 = time.time()
    ...
    lines_out.append("gametitle=Naruto UN5")
    lines_out.append("")

    lines_out.append("; codigo injetado")
    first_addr = min(obj_addrs.values())
    for name in objs:
        addr = obj_addrs[name]
    ...
    for org_addr, token, type_ in word_orgs:
        if type_ == "literal":
            ...
            if resolved is None and token in header_symbols:
                ...
                source = "Main.h"
            if resolved is None:
                print(f"  AVISO: símbolo '{token}' não encontrado (nem nos .o, nem no {HEADER_FILE})")
            else:
    ...
    print(f"Pnach gerado em {PNACH_FILE}")
