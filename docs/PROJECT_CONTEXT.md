...
- `na2_patcher/`: profile schema v2, reusable hash-pinned feature packages, ordered module orchestration, hash-pinned module data, and the translation/binary-patcher/Texture-patcher/external-translation/disc-identity implementations. `na2_patcher/profiles/current/` only enables and pins feature packages plus ordered module instances; each package under `na2_patcher/features/` owns its module selections, and current binary-patcher features use groups only. Binary-patcher, feature, and Texture-patcher hashes exclude adjacent READMEs and engine code; the complete committed checkpoint pins the integrated implementation. The disc-identity module runs last and performs the declared equal-length `SLPS_258.37` to `SLPS_222.28` boot-path rename.
