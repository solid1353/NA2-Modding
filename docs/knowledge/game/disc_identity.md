...
NUN6 is version A35, recorded with CRC `688C83D6`.
...
confirmed this for the unknown `SLUS-55606`: `NUN6.iso` appears as
`NUN6`. Because normal project images are intentionally named
...
can retain `Narutimate Accel v2.28 - Latest` or `NUN6`. A direct
...
`SLUS-55606 [?]` for NUN6 and `SLPS-22228 [?]` for an earlier modified
...
`crc =` metadata limits individual groups. On 2026-07-24 the retained Current,
