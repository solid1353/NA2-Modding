...
- `na2_patcher/modules/string_patcher/`: the reusable semantic string-placement engine. Localization-owned declarations live under `na2_patcher/features/localization/string_patcher/`; the engine merges them with runtime imports, omits current `[S]` fallback writes in favor of one compact resident MOD pool, compiles one in-memory binary-patcher package, and delegates byte guards, conflict validation, composition, and logging to `binary_patcher`.
