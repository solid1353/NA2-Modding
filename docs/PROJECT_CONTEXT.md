...
- `na2_patcher/modules/string_patcher/`: the semantic fixed-size string-replacement module. It stores only text-oriented declarations in `strings.tsv`, compiles them into an in-memory binary-patcher package, and delegates selection, byte guards, conflict validation, composition, and logging to the shared `binary_patcher` engine.
