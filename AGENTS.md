...
- Generated/intermediate files go under `build/`, `logs/`, `scripts/`, or `work/temp/` with task-named subfolders when throwaway workspace is needed. Original-source extractions are the only exception and stay beside their source archive under `source/` as `<archive filename>.files`.
