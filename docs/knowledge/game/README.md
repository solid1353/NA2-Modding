# Game knowledge

This domain owns game identity, startup behavior, asset families, and the
structure of the original disc and its contained files.

- [Disc identity](disc_identity.md)
- [Startup sequence](startup.md)
- [Save-data record format and lifecycle](save_data.md)
- [Resident front-end and mode flow](mode_flow.md)
- [Character assets](character_assets.md)
- [Character Select support list](character_select.md)
- [Content availability and save-backed unlock state](content_availability.md)
- [Frontend Shop and bonus game](frontend_shop.md)
- [Game files](files/README.md)
