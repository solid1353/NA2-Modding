...
if ($Help) {
    ...
    @(
        ...
        '      Select the newest NA2_APPLY__<SOURCE>__*.zip for every source.'
        '      Examples:'
        ...
        '  Every build validates all selected packages first, recreates the output'
        '  from a complete source ISO copy, then applies the selected packages.'
        '  Package source names control selection only; package contents may target'
        '  any valid source-ISO files. Cross-package path overlap is rejected.'
