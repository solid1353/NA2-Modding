...
- For a selected task, only read-only inspection is allowed before plan approval. An unambiguous use of `approved`, `qwe`, or the keyboard-layout equivalent of `qwe` produced by the same physical keys under another active input layout authorizes changes and may appear within a longer message. A second unambiguous approval after result review authorizes task deletion, commit, and push.
...
14. Another unambiguous `approved`, `qwe`, or keyboard-layout equivalent of `qwe` as defined above, including within a longer message, authorizes completed-task deletion, committing implementation or result changes, and push. Task-management-only changes already committed under rule 8 do not require another commit.
