...
2. If game logic changed, finish all earlier selected static and unit checks,
   then build a task-owned worker ISO through the existing worker build command.
3. Replay the same recording against the worker ISO into a task-owned candidate
   capture path through the exact command in the
...
4. Compare the task-relevant candidate captures with the baseline and report
   the observed result. Delete the disposable worker ISO after evidence
   extraction whether validation passes or fails.
5. If game logic did not change, do not build a worker ISO or replay a candidate
   merely because the recording was provided.

The candidate replay is agent validation, not user acceptance. Stage and report
the result under the active interaction mode; the user reviews or tests it and
accepts it with `ver`.
