...
- Former PNACH sections preserved as binary-patcher patch sets are `Testing`, `Rendering`, `QoL`, and `Battle logic`. Binary-patcher schema v2 organizes each package as groups, atomic patches, and exact edits; `default_enabled` preserves historical state. Rendering is currently an empty feature folder omitted from the active profile.
