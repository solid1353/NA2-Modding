...
presentation are shared with Battle Settings.
...
The shared renderer derives a visible-row mask from the current animated scroll
position. It submits label, value, and backing work only for rows inside the
visible region or its two-row transition margin, while the native loop still
advances every row's layout position. Without this culling, the 26-row Items
Settings page grows each of the two alternating packet lists from about 321 KiB
to 874 KiB and, together with the other render allocations, exhausts the native
