...
- Current verified working-build snapshot: `build/Current.iso`, size `1,928,429,568`, SHA-256 `2614C6B7D71C4D9BAF39D091FC4194F16F70D0007C5761C72BB9320239BBAD2C`, boot ELF CRC `08983B71`. It composes frozen font milestone m01, runtime-proven raw patch `ELF-M008`, and immutable translation m03/v33 through the current profile. The cleaned-tree profile build matches this ISO byte-for-byte. The active PNACH alias is `cheats/SLPS-25837_08983B71.pnach`.
...
- `build/`: normally contains `build/Current.iso`. Temporary, parity-check, and hypothesis-test ISOs may also remain while they have a concrete future testing or comparison use, but are permanently deleted as soon as they become useless.
- `milestones/packages/`: tracked frozen copies of explicitly accepted reusable compatibility artifacts. There is no top-level `packages/` staging/history directory; temporary imported archives live under task-specific `work/temp/` folders until normalized or retired. Every milestone filename uses a milestone identifier (`mNN`) before its timestamp; a distinct source/builder version (`vNN`) follows the timestamp when applicable.
- `na2_patcher/modules/translation/mappings.tsv` is the live translation workspace and `na2 tr` remains a standalone TSV export with a JSON build summary under `logs/na2_patcher/translation_exports/`. Reproducible builds use immutable mapping snapshots under `na2_patcher/milestones/`; the current profile references translation m03/v33. Older builder-archive milestones m01/v25 and m02/v30 were retired after exact profile parity; m03/v33 remains the sole legacy archive fallback under `milestones/packages/`.
...
- `na2_patcher/modules/raw_binary/`: repository-owned schema v1, CLI validator/patcher, normalized disabled GPT candidates, and verified historical font ELF patch groups. It never applies `pending`, `runtime_failed`, or `deprecated` patches and writes only new same-size outputs with complete logs. The main ISO compositor can apply proven raw patches after ZIP packages and before translation, with staged-byte conflict checks.
- `na2_patcher/`: profile schemas, ordered module orchestration, immutable module-data snapshots, and the translation/raw-binary implementations. `na2_patcher/profiles/current/` pins font m01, runtime-proven `ELF-M008`, and translation m03/v33 by exact hashes. A full profile build is byte-for-byte identical to the verified `build/Current.iso` SHA-256 `2614C6B7D71C4D9BAF39D091FC4194F16F70D0007C5761C72BB9320239BBAD2C`.
