...
def _patcher_files(patcher: Path) -> list[Path]:
    if not patcher.is_dir():
        raise FileNotFoundError(patcher)
    return sorted(
        (
            path
            for path in patcher.rglob("*")
            if path.is_file()
            and "__pycache__" not in path.relative_to(patcher).parts
            and path.suffix.casefold() not in GENERATED_SUFFIXES
        ),
        key=lambda path: path.relative_to(patcher).as_posix(),
    )


def patcher_tree_entry(patcher: Path) -> dict[str, object]:
    digest = hashlib.sha256()
    files = _patcher_files(patcher)
    if not files:
        raise ValueError("na2_patcher contains no fingerprintable files")
    total_size = 0
    for path in files:
...
def collect_build_state(
    *,
    ...
    patcher = (workspace / "na2_patcher").resolve()
    try:
        profile = profile_directory.relative_to(patcher).as_posix()
    except ValueError as error:
        raise ValueError("Profile must be inside na2_patcher") from error
    ...
    return {
        ...
        "patcher_tree": patcher_tree_entry(patcher),
        "profile": profile,
