...
- Former PNACH sections preserved as binary-patcher patch sets are `Testing`, `Rendering`, `QoL`, and `Battle logic`. Binary-patcher schema v2 organizes each package as groups, atomic patches, and exact edits; `default_enabled` preserves historical state. Rendering is currently an empty disabled feature and module in the active profile.
...
- `na2_patcher/modules/translation_importer/mappings.tsv` is the translation importer's current hash-pinned v35 input. The current profile imports and validates all official-string targets first; `string_patcher` then compiles the selected imported groups with its local declarations and delegates their guarded writes to `binary_patcher`. No standalone export or source-hash bypass exists.
...
- `na2_patcher/modules/binary_patcher/`: repository-owned schema v2, CLI validator/patcher, and canonical group/patch/edit packages for Font, menu input, QoL, battle logic, testing, UI translation code, and reserved Rendering. The schema has no relations table; selected edits are simulated in deterministic order so compatible overlaps and already-satisfied replacements are retained while real guard conflicts fail before ISO staging. It never applies `pending`, `runtime_failed`, or `deprecated` patches and writes only new same-size outputs with complete logs.
- `na2_patcher/modules/translation_importer/`: the official-string import boundary. It reads the hash-pinned mapping table and canonical NA2/NUN5 sources, validates and derives semantic import rows, and performs no game-payload writes.
- `na2_patcher/modules/string_patcher/`: the semantic fixed-size string-replacement module. It stores only local text-oriented declarations in `strings.tsv`, merges them with runtime import rows from `translation_importer`, compiles one in-memory binary-patcher package, and delegates selection, byte guards, conflict validation, composition, and logging to the shared `binary_patcher` engine.
- `na2_patcher/modules/texture_patcher/`: hash-pinned, source-derived fixed-size CCS container imports from the official NUN5 donor. It stores no replacement blobs: 33 whole strategies derive complete donor payloads, including `HOME.CCS` and `MAPSEL1.CCS`, while mapped `MODE2KDV.CCS` preserves its portrait/palette/lower rows. Profile hashes cover the three canonical TSV recipes, not generated replacements, parser code, or documentation.
- `na2_patcher/`: profile schema v2, reusable hash-pinned feature packages, ordered module orchestration, hash-pinned module data, and the translation-importer/binary-patcher/string-patcher/Texture-patcher/external-translation/disc-identity implementations. `na2_patcher/profiles/current/` only enables and pins feature packages plus ordered module instances; each package under `na2_patcher/features/` owns its module selections, and current binary-backed features use groups only. Binary-patcher hashes cover its five control tables and referenced blobs; string-patcher hashes cover only `strings.tsv`; feature and Texture-patcher hashes likewise exclude adjacent READMEs and engine code. The complete committed checkpoint pins the integrated implementation. The disc-identity module runs last and performs the declared equal-length `SLPS_258.37` to `SLPS_222.28` boot-path rename.
