...
- `logs/`: generated records grouped into task-specific subfolders. Current groups are `na2/`, `font/`, `translation/`, `packages/`, `milestones/`, `extraction/`, `inventory/`, and `trash/`; no files should be written directly in the `logs/` root. `na2` keeps only `logs/na2/latest.log` for the most recent completed run and `logs/na2/rolling.log` for cumulative history, with runs separated by human-readable timestamps.
