...
- `na2_patcher/features/localization/translation_importer/mappings.tsv` is the Localization feature's current hash-pinned v35 importer input. The current profile imports and validates all official-string targets first; `string_patcher` then chooses inline or compact external placement, compiles the resulting writes with its local declarations, delegates guarded edits to `binary_patcher`, and returns the compact `PRG/228.BIN` insertion to the compositor. No standalone export or source-hash bypass exists.
...
- `na2_patcher/modules/string_patcher/`: the reusable semantic string-placement engine. Localization configuration lives under `na2_patcher/features/localization/string_patcher/`; its local `strings.tsv` is currently header-only, while the engine compiles imported strings and compact external placement into one in-memory binary-patcher package and delegates byte guards, conflict validation, composition, and logging to `binary_patcher`. The memory-card title belongs to profile `identity.tsv`, not Localization.
