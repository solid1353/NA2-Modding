...
- **Assigned scope:** the clean NA2 battle status/buff/debuff and item-effect
system: application, per-fighter storage, authored definitions, requested and
stored countdown behavior, same-ID replacement, coexistence, update/expiry,
explicit removal and cleanup, item-driven effect provenance, and the boundary
between gameplay truth and status presentation. The owned output was this new
canonical document only; no existing knowledge file or index was edited.

- **Exploration depth:** the following areas were covered exhaustively within the stated static/direct
boundaries:

- all 138 resident effect-definition records for IDs `0x00..0x89`, including
  record boundaries, categories, base countdown distribution, flags,
  presentation selectors, non-null display descriptors, and the complete
  19-entry specialized-constructor registry;
- the generic resident lifecycle rooted at `FUN_00305C30`, `FUN_00305270`,
  `FUN_003059B0`, `FUN_00304D60`, and `FUN_00305040`, including the main and
  auxiliary per-fighter lists, every registered specialized vtable's
  constructor/destructor and slot-`+0x10` return behavior, the direct bulk
  cleanup call graph, and every direct resident exact-ID removal JAL;
- every direct JAL to `FUN_00305C30` in the clean resident executable and BTL
  overlay. Table-driven dispatch, route recursion, wrappers, and callers with
  dynamically supplied IDs were separated from literal-ID mappings; this
  completeness statement does not include indirect or runtime-generated calls;
- all `0x74` resident item-metadata records (`0x00..0x73`), every non-`-1`
  metadata direct-effect field, all `0xB6` BTL object-definition leading codes,
  and the pickup, selected-use, delayed-use, hit-carried, and three-slot
  inventory paths that consume those values;
- every byte of the 19-row, `0x2F8`-byte BTL three-slot status table, including
  the four effect-ID lanes versus the interleaved words the dispatcher does not
  read, all recovery branches, inventory consumption, and the byte-identical
  NUN5 comparison span;
- the exact BTL generic-list helpers used by the resident effect containers,
  checked in raw instructions where the preserved Ghidra export has undefined
  gaps or shifted partial functions.

Status-presentation coverage was bounded rather than a full battle-UI audit. It
covered the complete 12-row resident effect-to-notification map, complete
13-row auxiliary visual-selector map, Fukidasi factory dispatch codes
`0x00..0x14`, the numeric/single/paired authored UI-record maps used by those
objects, and the independent list lifecycles needed to prove that presentation
is not gameplay ownership. Rendering, animation internals beyond removal
conditions, and localization semantics were not pursued.

Character- and battle-controller behavior surrounding status calls was
sampled. Direct application/removal callsites and literal numeric requests were
catalogued exhaustively, but the wider behavior of every character-specific
caller was not reverse-engineered. Detailed follow-through was limited to
representative or lifecycle-relevant cases such as effects `0x22/0x23`,
`0x4A`, `0x54`, `0x7D`, the fighter-`0x57` effect-`0x09` toggle, caller-managed
effect families, and the item/object paths documented below.

- **Confirmed coverage:** the exact binary identities and overlay-address
conventions; definition, fighter, node, inventory, auxiliary, and notification
layouts; normal same-ID replace-versus-different-ID coexistence rules; `-1`,
`-2`, and zero-countdown behavior; countdown normalization and gated expiry;
removal-reason control flow and cleanup mutation order; linked/local routing
and its non-transactional behavior; direct item/effect mappings and requested
countdowns; the delayed dual path for item `0x5B`; effect-`0x22`'s
destructor-driven successor; and the separation of gameplay membership from
cache, inventory, auxiliary visuals, and Fukidasi notifications. Exact
offsets, resident runtime/file pairs, BTL file/Ghidra/live triples, callers,
callees, and strong negative results accompany those findings.

- **Unresolved or untested:** user-facing meanings for most numeric effect IDs and
removal reasons, gameplay names for most definition payload fields and the
higher-level meanings of non-routing flag predicates, the exact engine-phase
name of the secondary callback pass, a proven consumer for the authored
`0x65/0x12` interleaved pair, ordinary-runtime reachability of the six-code
selected-use fallback, and any indirect/dynamically constructed application or
removal calls. Most surrounding character-specific state machines also remain
unexplored beyond their status-facing edges.

- **Deliberate exclusions and overlap:** Adventure, substitution, damage scaling/formulas, 60-FPS or broader timing
work, widescreen/rendering, media, and localization were deliberately excluded.
Status UI was followed only far enough to establish ownership, mapping, and
lifetime boundaries, avoiding overlap with the dedicated battle-UI knowledge.

- **Evidence limitations:** all conclusions in this pass are static clean-binary results. No gameplay
execution, savestate trace, runtime injection, or empirical countdown timing
was performed. Raw instruction checks resolve export gaps and the NUN5 table
match provides cross-version corroboration, but neither substitutes for NA2
runtime validation. Consequences described for unusual states such as
externally duplicated nodes, effect-`0x22` mutation during hard cleanup, or
repeated effect-`0x7D` replacement are therefore code-path results, not observed
runtime captures.
...
| Binary | Size | SHA-256 |
| --- | ---: | --- |
| Clean `SLPS_258.37` | `5,273,256` | `20C0A40D70EA412CD431993A2E189B37ECB6054D63AE93BE545470016E1627AF` |
| Clean `PRG/BTL.BIN` | `2,237,184` | `56FD042740221E3CC91417194F147142799D51FE70642273F4E97BD389D5D63C` |

The resident ELF's first load segment maps file `0x100` to EE
`0x00100000`. For a resident virtual address `A`:

```text
SLPS file offset = A - 0x00100000 + 0x100
```

Live memory retains BTL's complete `0x40`-byte MWo3 header. The preserved
Ghidra import omitted that header. For an overlay-local complete-file offset
`x`:

```text
live EE address       = 0x006B3F00 + x
Ghidra/export address = 0x006B3EC0 + x
live                  = Ghidra/export + 0x40
```

Absolute MIPS pointers and J/JAL targets encoded in BTL are already live
addresses. They must not receive another `+0x40`. This matters because Ghidra
can attach a shifted or overlapping symbol to such a target. BTL data below is
therefore normally identified by complete-file offset and live address, not by
the misleading physical location in the headerless import.

One concrete trap is the callback at BTL file `0x8BDC0`, preserved as
`FUN_0073FC80` and live at `0x0073FCC0`. A live target `0x0073FC90` refers to a
different small helper; the export's overlapping `FUN_0073FC90` label is not
the callback entry.

Source evidence was checked in the clean binaries and the read-only exports
under `@disassembly/NA2/exports/BTL.BIN` and
`@disassembly/NA2/exports/SLPS_258.37`. BTL undefined-byte gaps were checked
against raw little-endian instructions rather than inferred from truncated C
output.
...
## Representative BTL application mappings
...
## Strong negative results and open questions

- Gameplay truth is live membership in fighter `+0x8C4/+0x8C8`, not
  `+0x8E8`, an auxiliary visual, an inventory count, or a UI notification.
- Same-ID application replaces through destruction; it does not refresh in
  place or increment a stack. Different IDs coexist.
- No generic category-wide exclusivity rule was found.
- No definition defaults to protected sentinel `-2`; known inherent callers
  supply it explicitly.
- Countdown values must not be labelled frames or seconds. The countdown pass
  is gated, low IDs can be normalized, and IDs `0/1` have an extra decrement.
- Registered gameplay-effect vtable `+0x10` callbacks do not return a proven
  self-remove signal in clean NA2. Node base-flags bit `0` remains a generic
  non-countdown removal mechanism.
- Effect `0x22`'s successor is destructor-driven and therefore not restricted
  to natural expiry.
- The three-slot table's interleaved words are not applied effects and are not
  read by its dispatcher. In particular, `65/12` does not prove an effect-to-UI
  route.
- Existing broad enumerations that treat interleaved values `06`, `11`, or
  `12` as effects selected by `FUN_007116E0` conflict with the raw loop and
  clean row bytes.
- Most definition payload fields, non-routing flag bits, semantic names for
  removal reasons, and many specialized effect behaviors remain unresolved.
- No runtime execution was performed for this research pass.

NUN5's homologous `0x2F8`-byte three-slot table at complete-file/live
`0x1EDBD0/0x008B48D0` is byte-identical, providing strong static corroboration
of the row layout. Its dispatcher is file/Ghidra/live
`0x60320/FUN_00726FE0/0x00727020` and calls resident homolog
`FUN_00310580`. That cross-version match does not replace NA2 runtime
verification.
