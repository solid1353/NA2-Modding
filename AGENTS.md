...
- The task workflow and its approval gates apply only to selected work from `TASKS.md`. Perform small, direct, low-risk changes immediately without a plan, intelligence-level recommendation, or approval gate; keep non-documentation changes uncommitted in the current changeset until the user requests or approves Git operations. Automatic commit and push applies only to selected `TASKS.md` tasks, subject to the documentation-only exception below.
...
- Commit documentation-only changes automatically without `qwe` or any other approval, using the authoring agent's identity, but do not push merely for those changes. This exception includes `AGENTS.md`, `TASKS.md`, plans, handoffs, and other documentation-only commits, including documentation-only selected-task results. A later push by another agent may include any already-present local commits; do not exclude or rewrite them.
...
8. After any authorized task-management edit to `TASKS.md`, commit that task update immediately without requesting approval, but do not push merely for this documentation-only change. Stage only `TASKS.md` and any dedicated task context or plan document created or edited specifically as part of the same task-management update; never include concurrent implementation or unrelated work. Run the required chat actualization before this automatic commit when a workstream subsection changed. Deleting a completed task still requires result approval before the deletion is made. A later in-scope push may include this existing commit.
...
13. When its work is complete, the agent refreshes Git state, stages only its intended changes, commits with its own registered identity, and reports the result. Push without requesting approval unless the resulting commit is documentation-only; documentation-only commits remain local until included by a later in-scope push.
14. Another unambiguous `approved`, `qwe`, or keyboard-layout equivalent of `qwe` as defined above, including within a longer message, authorizes deletion of the completed task. Commit that deletion automatically under rule 8 without pushing merely for the deletion.
...
- Any worker that works with files must first establish a uniquely named owned directory under `work/`, identified by the worker or its Codex task, and keep that task's copies, experiments, and intermediate artifacts inside it. Use subfolders such as `base/`, `mod/`, and `temp/` when useful. A worker may read another worker's directory but must not modify, move, rename, or delete anything there; to make changes, copy the needed file into its own directory and modify only that copy.
- Treat external files that may change during the task as unstable inputs. When they are reasonably small, including screenshots and savestates, copy them into the worker's owned directory before relying on or modifying them. Read very large external inputs in place when copying would be unreasonable, while following all source and original-media protections.
- Use temporary subfolders inside the worker's owned directory for throwaway/intermediate work. At task completion, clean the entire owned directory: delete disposable copies, probes, and generated files; promote reusable confirmed findings into canonical documentation or module data; and retain only artifacts with concrete future value. Every retained artifact must be mentioned in a nearby README, the relevant task/knowledge document, or another canonical inventory with its purpose and location so later workers can find and assess it.
- Use `work/` for project-specific reverse-engineering and binary-mod work. Keep work separated by worker and then target/task, for example `work/<worker>/<target>/base/` and `work/<worker>/<target>/mod/`. Keep reusable Ghidra projects and disassembly exports shared between related repositories under `@analysis/disassembly/<target>/`.
