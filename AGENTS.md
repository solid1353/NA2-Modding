...
- Do not hardcode stable project directories, canonical files, or repeated project paths in scripts, configuration, maintained commands, or generated metadata. Treat `project-paths.json` as their single source of truth: add or update the named root/file there first, then resolve it through the shared PowerShell or Python path loader in every consumer. A stable path migration should normally require changing the manifest rather than searching and rewriting consumers. Hardcoded paths are allowed only for transient/generated/caller-supplied values, test-fixture-local paths, or genuinely local one-off values that do not represent project configuration. Documentation refers to configured roots as `@root/...`.
...
5. When an approved completed task is deleted and that leaves its workstream subsection empty, move the whole empty subsection to `Archive` as part of the same completion update; never delete the subsection. If a task later returns, move the subsection out of `Archive`: to `In Progress` when that task is approved for active work, or to `Backlog` only when the user explicitly directs it there. Keep the coordinator chat available but unpinned; do not archive the Codex chat merely because its workstream is under `Archive`.
