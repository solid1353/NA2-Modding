...
- Use `work/` outside `work/temp/` for project-specific reverse-engineering and binary-mod work. Keep work separated by target/task, for example `work/<target>/base/` and `work/<target>/mod/`. Keep reusable Ghidra projects and disassembly exports shared between related repositories under `@analysis/disassembly/<target>/`.
