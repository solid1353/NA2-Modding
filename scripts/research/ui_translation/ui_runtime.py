...
from na2_patcher.project_paths import (  # noqa: E402
    ProjectPaths,
    load_project_paths,
    resolve_alias,
)
...
def rendering_for_target(
    ...
    return inspect_rendering_settings(
        paths.path("pcsx2", "inis", "PCSX2.ini"),
        paths.path("pcsx2", "gamesettings", target.settings_file),
    )
...
def capture_state(
    ...
    *,
    ...
    state_root = paths.path("pcsx2", "sstates")
...
def _manual_state_path(paths: ProjectPaths, target: Target, slot: int) -> Path:
    ...
    return paths.path(
        "pcsx2", "sstates", f"{target.serial} ({target.crc}).{slot:02d}.p2s"
    )
